...
本文档是基于 ReproForge P0 基线生成的历史题库，适用于校招、中高级和
高级/专家候选人。每套题设计为约 60 分钟的实际问答。当前项目已经完成 P1，
因此涉及“当前”的题干和数字仍按 P0 快照作答；P1 的最新实现和验证请另见
[P1 技术参考](P1-TECHNICAL-REFERENCE.md)。P2–P8 仍不得当作现成功能。
...
P0 基线当时的 25 个测试和较高覆盖率还缺少哪些关键场景？请区分单元、契约和集成测试，并说明为什么这个历史数字不能代表当前 P1 的全部验证。
...
- 混淆题库的 P0 历史快照、当前已完成的 P1，以及仍在规划的 P2–P8。
